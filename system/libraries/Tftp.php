...
//        return FALSE;
    ...
    public function upload_images_to_ftp_server($config, $new_width = FALSE, $new_height = FALSE) {
        ...
        $config_local['max_size'] = $config['upload']['max_size'];
        $config_local['max_width'] = $config['upload']['max_width'];
        $config_local['max_height'] = $config['upload']['max_height'];
        $config_local['file_name'] = $config['upload']['file_name'];
        $config_local['overwrite'] = $config['upload']['overwrite'];
        // initialize
    ...
    public function is_file_exist($config, $params) {
        ...
        if ($this->connect($connect)) {
            ...
            $list_file = empty($list_file) ? array() : $list_file;
...
//        return FALSE;
    ...
    // upload file on local to server
    public function move_file_local_to_ftp_server($config, $params) {
        /*
          dir_path : tujuan
          file_name : tujuan
          local_file : source
        ...
        if ($this->connect($connect)) {
            ...
            // file local
            $local_file = trim($params['local_file'], '/');
            // remote files
            $remote_file = trim($params['dir_path'], '/') . '/' . trim($params['file_name'], '/');
            if ($this->upload($local_file, $remote_file, 'binary', $config['ftp']['ftp_chmod'])) {
                return TRUE;
            } else {
                $this->tupload->set_error('upload_failed');
                return FALSE;
